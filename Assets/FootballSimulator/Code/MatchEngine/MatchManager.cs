...
namespace FStudio.MatchEngine {
    public class MatchManager : SceneObjectSingleton<MatchManager> {
        //^^7 스테이지 끝 표현을 위한 -> update에서 사용
        public bool screenshotEnd = false;
        public bool stageEnd = false;
        public bool stage_1 = false; //^^7 613 stage몇인지 구별하기 위함
        ...
        public static async Task CreateMatch(
            ...
            Current.stageEnd = false; //^^7 stageEnd 세팅
        ...
        private async Task CreateMatch (
            ...
            //ball.ResetBall(midPoint); //^^7 공 위치 조건부로 수정
            var stage1Point = new Vector3(fieldEndX * 0.75f, 0, fieldEndY * 0.9f);
            var stage2Point = new Vector3(fieldEndX * 0.24f, 0, fieldEndY * 0.5f);
            ...
            //^^7 킥오프 위치 세팅 -> 팀1이 stage1인경우 
            if (team1.Team.TeamName == "stage1") //team1.Team.TeamName == "stage1"
            {
                stage_2 = false;
                ball.ResetBall(stage1Point);
                PrePositionTeamsForStage1();
                stage_1 = true;

                for (int i = 0; i < 11; i++)
                {
                    Debug.Log("Color Changingggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg");
                    GameTeam1.GamePlayers[i].PlayerController.CUI.SetColor(); //^^7 과연 1팀만 색이 바뀔까? -> 바뀐다!
                    GameTeam2.GamePlayers[i].PlayerController.CUI.SetColor2();
                }
            }               
            else if (team1.Team.TeamName == "stage2")
            {
                stage_1 = false;
                ball.ResetBall(stage2Point);
                PrePositionTeamsForStage2();
                stage_2 = true;

                for (int i = 0; i < 11; i++)
                {
                    GameTeam1.GamePlayers[i].PlayerController.CUI.SetColor();
                    GameTeam2.GamePlayers[i].PlayerController.CUI.SetColor2();
                }
            }              
            else
        ...
        private void RandomizeKickOff () {
            ...
            whichTeamStarted = 1; //항상 공격자 팀이 시작하도록
        ...
        public void PrePositionTeamsForKickOff () {
            ...
            //^^7 stage1테스트 -> PrePositionForStage1 함수를 새로 만드는 것으로 변경
            if (homeTeamStart) {
        ...
        public void PrePositionTeamsForStage1()
        {
            Debug.Log("[MatchManager] PrePositionTeamsForStage1");
            ...
            //^^7 stage1테스트
            GameTeam1.PrePositionForStage1(
                    fieldEndX,
                    fieldEndY,
                    goalNet1,
                    Vector3.forward,
            ...
            GameTeam2.PrePositionDefensiveStage1(
                 in fieldEndX,
                 in fieldEndY,
                 goalNet2);
        ...
        //^^7 stage2
        public void PrePositionTeamsForStage2()
        {
            Debug.Log("[MatchManager] PrePositionTeamsForStage2");
            ...
            //^^7 stage2
            GameTeam1.PrePositionForStage2(
                    fieldEndX,
                    fieldEndY,
                    goalNet1,
                    Vector3.forward,
            ...
            GameTeam2.PrePositionDefensiveStage2(
                 in fieldEndX,
                 in fieldEndY,
                 goalNet2);
...
#region GK Condition
        ...
        private void LateUpdate() {
            ...
            if (MatchFlags == MatchStatus.Playing) {
                ...
                //^^7 613 골이 들어가거나 상대가 공을 뺏은경우 경기종료
                if ((ball.LastTouchedPlayer.GameTeam == GameTeam2)||(goalNet2.GoalColliders == true || goalNet1.GoalColliders == true || goalNet2.OutColliders == true || goalNet1.OutColliders == true)) //ball.LastTouchedPlayer.GameTeam == GameTeam2 //0611 ball.HolderPlayer?.GameTeam == GameTeam2
                { 
                    stageEnd = true;
                    ball.SaveLogToJson();
                    //@@@@@@ 경기 시간 관련 오류 해결 필요
                ...
                //^^7 겟키업 테스트 - stageEnd 다시 false로 돌려놓기 6/7 테스트용으로
                if (!matchShouldContinue||stageEnd==true) {
                    //stageEnd=false;
