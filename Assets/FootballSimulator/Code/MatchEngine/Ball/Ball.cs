    ...
    public partial class Ball : SceneObjectSingleton <Ball> {
        ...
        private void LateUpdate() {
            ...
            if (Time.time > nextRecordTime)
            {
                ...
                nextRecordTime = Time.time + 0.1f; // 0.1초 마다 그리기
            }
        }
        ...
        {
            string json = JsonUtility.ToJson(new BallPositionLog(ballPositions));
            File.WriteAllText(Application.persistentDataPath + "/ball_log.json", json);
        }
    }
}

[System.Serializable]
public class BallPositionLog
{
    public List<Vector3> positions;

    public BallPositionLog(List<Vector3> positions)
    {
